...
public final class Client {
    ...
    public func checkConnection() throws {
        try _ = basicCommand(command: ["ping": 1], databaseName: "local")
    }
    ...
    public func basicCommand(command: BSON.Document, databaseName: String) throws -> BSON.Document {
        ...
        guard let res = BSON.documentFromUnsafePointer(documentPointer: reply.pointer) else {
    ...
    public func getServerStatus() throws -> BSON.Document {
        ...
        guard let status = BSON.documentFromUnsafePointer(documentPointer: reply.pointer) else {
            throw MongoError.CorruptDocument
        }
