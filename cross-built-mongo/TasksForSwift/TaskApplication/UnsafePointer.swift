...
public extension BSON {
    ...
    public final class AutoReleasingCarrier {
        ...
        convenience init(doc: BSON.Document) throws {
            guard let bson = BSON.unsafePointerFromDocument(document: doc) else {
...
public extension BSON {
    ...
    static func unsafePointerFromDocument(document: BSON.Document) -> UnsafeMutablePointer<bson_t>? {
        ...
        for (key, value) in document {
            ...
            guard appendValue(documentPointer: documentPointer!, key: key, value: value) == true
                else { return nil }
        }
    ...
    static func documentFromUnsafePointer(documentPointer: UnsafePointer<bson_t>) -> BSON.Document? {
        ...
        guard iterate(document: &document, iterator: &iterator) == true
            else { return nil }
    ...
    static func appendValue(documentPointer: UnsafeMutablePointer<bson_t>, key: String, value: BSON.Value) -> Bool {
        ...
        case let .Code(code):
            ...
            if let scope = code.scope {
                ...
                guard let scopePointer = BSON.unsafePointerFromDocument(document: scope)
                    else { return false }
        ...
        case let .Document(childDocument):
            ...
            for (childKey, childValue) in childDocument {
                ...
                guard appendValue(documentPointer: childDocumentPointer!, key: childKey, value: childValue)
                    else { return false }
            }
        ...
        case let .Array(array):
            ...
            for (index, subvalue) in array.enumerated() {
                ...
                guard appendValue(documentPointer: childPointer!, key: indexKey, value: subvalue)
                    else { return false }
            }
        ...
        while bson_iter_next(&iterator) {
            ...
            case BSON_TYPE_DOCUMENT:
                ...
                guard bson_iter_recurse(&iterator, &childIterator) &&
                    iterate(document: &childDocument, iterator: &childIterator)
                    else { return false }
            ...
            case BSON_TYPE_REGEX:
                ...
                let optionsBufferPointer = UnsafeMutablePointer<UnsafePointer<Int8>?>.allocate(capacity: 1)
            ...
            case BSON_TYPE_CODEWSCOPE:
                ...
                guard let scopeDocument = documentFromUnsafePointer(documentPointer: &scopeBSON)
