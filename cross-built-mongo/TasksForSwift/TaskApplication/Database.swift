...
//    }
    ...
    public func createCollection(name: String, options: BSON.Document) throws -> Collection {
    ...
    public func findCollections(filter: BSON.Document) throws -> Cursor {

        let filter = try BSON.AutoReleasingCarrier(doc: filter)
        var error = bson_error_t()
